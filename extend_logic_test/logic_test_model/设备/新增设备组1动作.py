...
def extend_device_group1(page: Page, device_group1_name=None):
    if device_group1_name is None:
        device_group1_name = generate_random_device_group_name()
    ...
    with open(txt_file_path, 'r', encoding='utf-8') as file:
        main_device_group_name = file.read().strip()

    print("主设备组名称已成功读取:", main_device_group_name)

    ###
    extend_device_action(page)
    page.get_by_label("添加设备组").get_by_placeholder("请选择").click()
    page.get_by_label("添加设备组").get_by_placeholder("请选择").fill(main_device_group_name)
    page.get_by_text(main_device_group_name).click()
    page.get_by_label("添加设备组").get_by_placeholder("请输入组织名称").click()
    page.get_by_label("添加设备组").get_by_placeholder("请输入组织名称").fill(device_group1_name)
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # 构建 txt 文件相对路径
    txt_file_rel_path = os.path.join('..', '..', '..', 'data', 'extend_logic_data_collection', '设备', '设备组1.txt')
    # 构建 txt 文件绝对路径
    txt_file_path = os.path.join(script_dir, txt_file_rel_path)

    # 写入文件
    with open(txt_file_path, 'w', encoding='utf-8') as file:
        file.write(device_group1_name)

    print("设备组1名称已成功写入到文件:", txt_file_path)