...
def extend_device_group2(page: Page, device_group2_name=None):
    if device_group2_name is None:
        device_group2_name = generate_random_device_group_name()
    ...
    with open(txt_file_path, 'r', encoding='utf-8') as file:
        device_group1_name = file.read().strip()

    print("设备组1名称已成功读取:", device_group1_name)

    ###
    extend_device_action(page)
    page.get_by_label("添加设备组").get_by_placeholder("请选择").click()
    page.get_by_label("添加设备组").get_by_placeholder("请选择").fill(device_group1_name)
    page.get_by_text(device_group1_name).click()
    page.get_by_label("添加设备组").get_by_placeholder("请输入组织名称").click()
    page.get_by_label("添加设备组").get_by_placeholder("请输入组织名称").fill(device_group2_name)
    ...
    with open(txt_file_path, 'w', encoding='utf-8') as file:
        file.write(device_group2_name)

    print("设备组2名称已成功写入到文件:", txt_file_path)