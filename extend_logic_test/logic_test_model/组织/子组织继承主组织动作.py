...
def add_extend_organization_action(page: Page, current_orgname=None):
...
##
    ...
    with open(txt_file_path, 'r', encoding='utf-8') as file:
        main_org_name = file.read().strip()

    print("主组织名称已成功读取:", main_org_name)

##
    extend_organization_action(page)
    page.get_by_label("创建组织").get_by_placeholder("请选择").click()
    page.get_by_label("创建组织").get_by_placeholder("请选择").fill(main_org_name)
    page.locator("li").filter(has_text=main_org_name).locator("span").click()
...
###
    ...
    print("主组织下子组织已成功写入到文件:", txt_file_path)